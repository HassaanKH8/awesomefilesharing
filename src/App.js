...
const socket = io(process.env.REACT_APP_URL_LINK);
                ...
                <div className='sessionscontainer'>
                  {filesSentSuccessfully ? (
                    <div>
                      ...
                      <p style={{ fontFamily: "EB Garamond", fontSize: 16, color: '#ededed', marginTop: 5, textAlign: "center" }}>{sessionId}</p>
                    </div>
                  ) : (
                    <div style={{ width: "100%" }}>
                      <p style={{ textAlign: 'right', marginBottom: 5, fontFamily: "EB Garamond", fontWeight: 500, color: '#ededed' }}>{progress}%</p>
                ...
                <div>
                  {selectedFiles.length > 0 && selectedFiles.map((file, index) => (
                    <div key={index} style={{ display: "flex", flexDirection: "row", alignItems: "center", justifyContent: 'space-between', marginBottom: 5 }}>
                      ...
                      <img alt='delete' src={require('./delete.png')} style={{ width: 20, height: 20, cursor: 'pointer', filter:'grayscale(1)' }} onClick={() => handleFileDelete(index)} />
                    </div>
                  ))}
                </div>
              )}
                ...
                {receivePressed && (
                  <div>
                    <div style={{display: 'flex', flexDirection: 'row', justifyContent: "space-between", alignItems: 'center'}}>
                      {loading && (
                        <p style={{ marginBottom: 5, fontFamily: "EB Garamond", fontWeight: 500, color: '#ededed', fontSize: 18 }}>{`${loadingText}${'.'.repeat(dotCount)}`}</p>
                      )}
                      <p style={{ marginBottom: 5, fontFamily: "EB Garamond", fontWeight: 500, color: '#ededed', fontSize: 18 }}>{progress}%</p>
                    </div>
                    <ProgressBar progress={progress} />
                  </div>
                )}
